...
class UploadRouteTest extends FlatSpec with ScalatestRouteTest with Matchers with OneInstancePerTest with MockitoSugar {
  ...
  it should "return 500 response code in case of error during saving of uploading file locally" in {
    ...
    val erroneousRoute =
      new UploadRoute("/nonexistentFolder", appContext.duplicationCheckService, appContext.videoProcessingRouter)
  ...
  it should "return BadRequest response in case non-uniqueness of uploaded file and cleanup temp file" in {
    ...
    val duplicationCheckService = new DuplicationCheckService {
      override def doesAlreadyExist(fileHash: String): Future[Boolean] = Future.successful(true)
    }

    val erroneousRoute =
      new UploadRoute(tempFolder.toString, duplicationCheckService, appContext.videoProcessingRouter)
  ...
  it should "return 500 response code in case of error during video processing and cleanup temp file" in {
    ...
    val erroneousRoute =
      new UploadRoute(tempFolder.toString, appContext.duplicationCheckService, system.actorOf(videoProcessorMockProps))
